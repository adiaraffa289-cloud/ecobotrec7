...
class AuthManager {
   ...
   static checkAuth() {
    ...
    // Jika tidak login dan BUKAN di halaman login, baru lempar ke login
    if (!currentUser && !pathname.includes('login.html')) {
        window.location.href = 'login.html';
        return false;
    }
    
    // HAPUS atau KOMENTAR bagian ini agar tidak mental ke index terus:
    /*
    if (currentUser && pathname.includes('login.html')) {
        window.location.href = 'index.html';
        return false;
